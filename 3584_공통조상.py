...
그 트리 상의 두 정점이 주어질 때 그들의 가장 가까운 공통 조상(Nearest Common Anscestor)은 
다음과 같이 정의됩니다.
...
그 중 깊이가 가장 깊은(15와 11에 가장 가까운) 노드는 4 이므로 
가장 가까운 공통 조상은 4가 됩니다.

루트가 있는 트리가 주어지고, 두 노드가 주어질 때 그 두 노드의 가장 가까운 공통 조상을 
찾는 프로그램을 작성하세요
...
3
'''
# 첫 노드의 부모들을 모두 True로 체크해 준 뒤
# 두번쩨 노드의 출발 지점에서부터 부모들을 하나씩 check 해주고 처음으로 True 가 나오면
# 그것이 공통 조상인 것이므로 출력해줌
