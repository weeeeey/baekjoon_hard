...
소설의 모든 장을 쓰고 나서는 각 장이 쓰여진 파일을 합쳐서 최종적으로 소설의 완성본이 들어있는 
한 개의 파일을 만든다. 이 과정에서 두 개의 파일을 합쳐서 하나의 임시파일을 만들고, 
...
'''
# 굉장히 어려웠다.
# 일단 기준점을 잡는 방법을 몰라서 dp 문제인지도 캐치 못함
# 묶는 기준점에 따라 값이 변하므로 최적의 기준점(묶음)을 찾아야함
# 묶여졌을때의 최적의 해를 메모해둔다면 나중에 끄집어 낼 수 있으므로 메모레이션 필요 
# dp[i][j] = min(dp[i][k] + dp[k][j+1]) + C 가 되는 k를 찾아야함
# k의 범위를 i부터 j로 둘경우 시간 초과 뜸 O(n^3) 이므로 

# 문제가 되는 것은 k 를 찾는 방법이므로 
# 이것을 줄일 수 있어야함
# 크눅스 최적화에 따르면 k의 범위는 k[i][j-1] <= k[i][j] <= k[i+1][j] 이다.
# 구할려는 길이를 점점 늘리다보면 0 부터 n-1 까지의 dp값을 구할 수 있음
# 3줄 짜리를 구하면 4줄을 구할 수 있음.

# 크눅스 최적화가 성립하는 증명은 찾아볼 수 없었음 
...
while(num):
  ...
  for i in range(1,n+1):
    knuth[i-1][i] = i
    sum_t.append(sum_t[i-1]+arr[i-1])
  ...
  for length in range(2,n+1):
    for s in range(n-length+1):
      ...
      for k in range(knuth[s][e-1],knuth[s+1][e]+1):
        temp = dp[s][k]+dp[k][e] +sum_t[e]-sum_t[s]
        if ans>temp:
          ans = temp 
          knuth[s][e] = k 
      dp[s][e] = ans
